...
interface Product {
  ...
  name: string;
  price: string;
  image?: string; // Optional: can be replaced with icon for now
...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  return (
    <Card className="group overflow-hidden rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300">
      <div className="relative w-full h-56 bg-muted/10 flex items-center justify-center rounded-2xl">
        ...
        {/* <span className="text-muted-foreground">{product.image || "🛍️"}</span> */}
        <Image src={product.image || ""} alt="Product" width={300} height={300} className="object-contain rounded-2xl border" />
